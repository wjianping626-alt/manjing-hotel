...
function buildPrompt(params) {
  ...
  return `为${destination}规划一个${days}天旅行行程。用户偏好：${labels}。要求：安排在慢境·麻黄梁艺术家酒店住宿，行程紧凑但不赶，有节奏感，融入当地特色体验和用户兴趣偏好，每个时间段包含具体地点和活动描述，适当加入实用建议`;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!apiKey) {
      ...
      return res.status(200).json({ ok: true, demo: true, data: demo });
    }
    ...
    return res.status(200).json({ ok: true, data: tripData });
  } catch (err) {
